...
def _required_fields_for_series(series):
    ...
    for item in KOMGA_LIBRARY_LIST:
        if item.get("LIBRARY") == library_id:
            return item.get("REQUIRED_FIELDS", []) or []
    return []
...
def _series_needs_refresh(series, required_fields):
    ...
    for field in required_fields:
        if field == "thumbnail":
            if len(komga.get_series_thumbnails(series["id"])) == 0:
                return True
            continue
...
def _book_needs_refresh(book, required_fields):
    metadata = book.get("metadata") or {}
    for field in required_fields:
        if field == "thumbnail":
            if len(komga.get_book_thumbnails(book["id"])) == 1:
                return True
            continue
        value = metadata.get(field)
        if value is None or value == "" or value == [] or value == {}:
...
def refresh_book_metadata(subject_id, series_id, force_refresh_flag, required_fields=None):
    ...
    for book in books["content"]:
        ...
        if book_record and not force_refresh_flag:
            if book_record[2] == 1 and not _book_needs_refresh(book, required_fields or []):
                continue

...
createApp({
  ...
  methods: {
    ...
    newTask() { this.editingTask = { id: '', name: '', type: 'metadata_completion', fields: ['title', 'summary'], card_ids: [], enabled: true }; },
    editTask(task) { this.editingTask = { ...task, fields: [...(task.fields || [])], card_ids: [...(task.card_ids || [])] }; },
    async saveTask() { if (!this.editingTask) return; if (!this.editingTask.name.trim()) { this.notify('请填写自定义任务名称', true); return; } if (!this.editingTask.fields.length) { this.notify('请至少选择一个元数据项', true); return; } if (!this.editingTask.card_ids.length) { this.notify('请至少选择一个媒体库', true); return; } try { const data = await this.api('/api/tasks', { method: 'POST', body: JSON.stringify(this.editingTask) }); this.tasks = data.items || []; this.config.METADATA_TASKS = this.tasks; this.editingTask = null; this.notify('计划任务已保存'); } catch (error) { this.notify(error.message, true); } },

...
createApp({
  ...
  methods: {
    ...
    newTask() { this.editingTask = { id: '', name: '元数据补全', fields: ['title', 'summary'], card_ids: this.cards.filter(card => card.id).map(card => card.id), enabled: true }; },
    editTask(task) { this.editingTask = { ...task, fields: [...(task.fields || [])], card_ids: [...(task.card_ids || [])] }; },
    async saveTask() { if (!this.editingTask) return; try { const data = await this.api('/api/tasks', { method: 'POST', body: JSON.stringify(this.editingTask) }); this.tasks = data.items || []; this.config.METADATA_TASKS = this.tasks; this.editingTask = null; this.notify('计划任务已保存'); } catch (error) { this.notify(error.message, true); } },

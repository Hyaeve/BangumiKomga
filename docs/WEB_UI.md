...
账号密码保存在容器的数据目录 `/app/data/web_auth.json`，默认 compose 将其绑定到宿主机的 `./data`。不要删除该目录或使用不带数据卷的 `docker run`，否则容器重建后只能回到默认账号 `admin`、密码 `password`。升级时会读取并同步旧版 `/app/config/web_auth.json`。

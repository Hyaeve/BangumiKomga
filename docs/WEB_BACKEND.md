...
- 登录背景沿各列自身漂移方向从视口外移入：奇数列从上往下，偶数列从下往上，持续 12 秒，相邻列延迟 0.4 秒。不再对整列做透明度淡入，封面随移动依次进入画面。每列有图片加载成功后开始，后续加载的图片轻柔淡入，最终完全不透明。
- 浮现容器与持续漂动容器分离，保持原有封面比例、排列与加载失败重试。减少动态效果模式下直接显示，不播放浮现或漂动。

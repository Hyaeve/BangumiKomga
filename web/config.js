...
createApp({
  ...
  methods: {
    ...
    makeCard(item = {}, index = 0) { return { uid: `${Date.now()}-${Math.random()}`, id: item.LIBRARY || '', serverId: item.SERVER_ID || '', name: '', path: '', covers: [], isNovel: !!item.IS_NOVEL_ONLY, rules: item.REQUIRED_FIELDS || [], overwriteFields: item.OVERWRITE_FIELDS || [], hue: this.cardHues[index % this.cardHues.length] }; },

...
createApp({
  ...
  data() {
    return {
      ...
      cardFeatureOptions: [
        ...
        { value: 'loginBackground', label: '登录背景' }
      ],
      serviceModeOptions: [
  ...
  methods: {
    ...
    mixedFilterTask(task) {
      if (!task.filter_regex) return task;
      const terms = Array.isArray(task.filter_terms) ? task.filter_terms : String(task.filter_terms || '').split(/\r?\n/);
      return {...task, filter_regex:false, filter_terms:terms.map(term=>term.trim()).filter(Boolean).map(term=>`/${term}/`).join('\n')};
    },

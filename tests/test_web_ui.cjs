...
async function main() {
  ...
  try {
    ...
    assert(bottomActions.y>=0 && bottomActions.y+bottomActions.height<=1000);
    assert(await page.locator('.task-editor-backdrop').evaluate(el=>el.scrollTop>0));
    await page.screenshot({path:path.join(output,'task-filter-resized.png')});
    await filterInput.evaluate(el=>{el.style.height='144px';});
    await page.getByRole('button', { name: '元数据候选', exact: true }).click();
    ...
    await page.locator('.task-modal .modal-actions').scrollIntoViewIfNeeded();
    const mobileActions = await page.locator('.task-modal .modal-actions').boundingBox();
    assert(mobileActions.y>=0 && mobileActions.y+mobileActions.height<=844);
    await page.screenshot({path:path.join(output,'task-filter-mobile.png')});
    await page.getByRole('textbox',{name:'过滤词条',exact:true}).evaluate(el=>{el.style.height='144px';});
    await page.setViewportSize({width:1440,height:1000});
